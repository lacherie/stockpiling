...
class Item:
    ...
    def total_price(self) -> float:
        subprices = [accessory.total_price() * amount_needed for accessory, amount_needed in self.accessories]
...
class Pandemic:
    ...
    def dictionary_of_contacts(self,Profession,population) -> Dict{}:
        dictionary0 = {self.hospitalised_total(population),
               self.outpatient_visits(population),
               self.non_icu_patient_days(population),
               self.non_mv_icu_patient_days(population),
               self.mv_icu_patient_days(population)
...
class Profession:
    name: str
    contacts_per_hospitalised: int #0 for other HCW, only input for escort
    contacts_per_outpatient_visit: int #0 for resp tech, radio tech, housekeepers 
    contacts_per_non_icu_patient_day: int #0 for administrative, only input for phlebotomists
    contacts_per_icu_patient_day: int #0 for administrative
    contacts_per_mv_day: int #0 for administrative
...
if __name__ == "__main__":
    papr = Item(
        ...
        accessories=[
            ...
            (Item("Hood",30.87,Box(3,9.0,9.0,9.0),accessories=[]
            ),3),
            (Item("Tube",30.89,Box(5,20.0,16.0,10.0),accessories=[]
            ),3)
        ]
    )
    elastomeric = Item(
        ...
        accessories=[(Item(name="E Filter set", single_price=2.5, box= Box(1,5.3,5.1,1.4), accessories=[]),3)]
    )
    n95 = Item(
        name="N95",
        single_price=0.25,
